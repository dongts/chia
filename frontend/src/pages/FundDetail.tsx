...
const TX_TYPE_CONFIG = {
  contribute: { label: "Contribute", color: "text-primary", icon: "▲" },
  withdraw: { label: "Withdraw", color: "text-error", icon: "▼" },
  expense: { label: "Expense", color: "text-error", icon: "▼" },
  holder_change: { label: "Holder Changed", color: "text-tertiary", icon: "↔" },
} as const;
    ...
    <div className="space-y-6">
      ...
      {fund.contributions_by_member.length > 0 && (
        <div className="bg-surface-container-lowest rounded-2xl shadow-editorial p-4">
          <p className="text-[11px] font-semibold text-on-surface-variant uppercase tracking-wide mb-3">Contributions by member</p>
      ...
      {/* Transaction history */}
      <div>
        <p className="text-[11px] font-semibold text-on-surface-variant uppercase tracking-wide mb-3">Transaction History</p>
                ...
                <div key={tx.id} className="flex items-center justify-between px-4 py-3.5">
                  ...
                  <div className="flex items-center gap-2 ml-3 shrink-0">
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-outline">by {tx.created_by_name}</p>
                    </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-6 pt-6 pb-2">
              <h3 className="text-lg font-bold text-on-surface capitalize">{showModal}</h3>
              <button
                onClick={closeModal}
            ...
            <div className="px-6 pb-6 pt-3 space-y-4">
              <div>
                <label className="block text-xs font-medium text-on-surface-variant mb-1.5">Member *</label>
                <select
                  value={txMemberId}
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {submitting ? "Saving..." : showModal === "contribute" ? "Add Contribution" : "Record Withdrawal"}
                </button>
              </div>
            </div>
          </div>
